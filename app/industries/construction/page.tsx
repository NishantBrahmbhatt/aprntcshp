...
import {
  ...
  general,
  tradesAndSkilled,
} from "@/lib/data/industries/construction";

const SECTION_IDS = {
  general: "construction-section-general",
  architecture: "construction-section-architecture",
  trades: "construction-section-trades",
  electrical: "construction-section-electrical",
} as const;

const SECTION_PILLS: { id: string; label: string }[] = [
  { id: SECTION_IDS.general, label: "General" },
  { id: SECTION_IDS.architecture, label: "Architecture & Design" },
  { id: SECTION_IDS.trades, label: "Trades & Skilled Work" },
  { id: SECTION_IDS.electrical, label: "Electrical & Gas" },
];
...
function PageHeader() {
  return (
    <section className="pt-8 pb-10">
      <IndustryBreadcrumb industryLabel="Construction, Architecture & the Built Environment" />
      <div className="space-y-3">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold tracking-tight text-neutral-50">
          <span className="inline-block pb-1 bg-[linear-gradient(180deg,#ffffff_0%,#707070_100%)] bg-clip-text [-webkit-background-clip:text] text-transparent [-webkit-text-fill-color:transparent]">
            Construction, Architecture & the Built Environment
          </span>
        </h1>
        <p className="text-sm sm:text-base text-neutral-500 max-w-2xl">
          Everything you need to pursue a construction or architecture
          apprenticeship.
        </p>
      </div>
    </section>
...
function SectionHeading({ title }: { title: string }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <h2 className="scroll-mt-[72px] text-sm font-medium tracking-wide text-neutral-300">
        {title}
      </h2>
    </div>
  );
...
function ConstructionSectionsNav({
  activeSectionId,
  onPillClick,
}: {
  ...
  return (
    <nav
      ...
      aria-label="Construction, architecture and the built environment guide sections"
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-neutral-50">
      <main className="mx-auto flex min-h-screen max-w-7xl flex-col px-6 pt-5 pb-20">
        ...
        <div className="flex-1">
          ...
          >
            <SectionHeading title="Architecture & Design" />
            <div className="grid gap-5 md:grid-cols-2">
              {architectureAndDesign.map((r) => (
                <ResourceCard
                  key={r.href}
                  title={r.title}
                  source={r.source}
          ...
          <section
            ...
            className="scroll-mt-[72px] space-y-4 py-10"
          >
            <SectionHeading title="Trades & Skilled Work" />
            <div className="grid gap-5 md:grid-cols-2">
              {tradesAndSkilled.map((r) => (
                <ResourceCard
                  key={r.href}
                  title={r.title}
                  source={r.source}
          ...
          <section
            ref={electricalRef}
            id={SECTION_IDS.electrical}
            className="scroll-mt-[72px] space-y-4 pt-10"
          >
            <SectionHeading title="Electrical & Gas" />
            <div className="grid gap-5 md:grid-cols-2">
              {electricalAndGas.map((r) => (
                <ResourceCard
                  key={r.href}
                  title={r.title}
                  source={r.source}
