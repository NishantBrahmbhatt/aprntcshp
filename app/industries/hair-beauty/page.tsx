...
import { beauty, general, hair } from "@/lib/data/industries/hair-beauty";

const SECTION_IDS = {
  general: "hair-beauty-section-general",
  hair: "hair-beauty-section-hair",
  beauty: "hair-beauty-section-beauty",
} as const;

const SECTION_PILLS: { id: string; label: string }[] = [
  { id: SECTION_IDS.general, label: "General" },
  { id: SECTION_IDS.hair, label: "Hair" },
  { id: SECTION_IDS.beauty, label: "Beauty" },
];
...
function PageHeader() {
  return (
    <section className="pt-8 pb-10">
      <IndustryBreadcrumb industryLabel="Hair & Beauty" />
      <div className="space-y-3">
        ...
        <p className="text-sm sm:text-base text-neutral-500 max-w-2xl">
          Everything you need to pursue a hair or beauty apprenticeship.
        </p>
      </div>
    </section>
  );
...
function HairBeautySectionsNav({
  activeSectionId,
  onPillClick,
}: {
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-neutral-50">
      <main className="mx-auto flex min-h-screen max-w-7xl flex-col px-6 pt-5 pb-20">
        ...
        <div className="flex-1">
          ...
          <section
            ref={beautyRef}
            id={SECTION_IDS.beauty}
            className="scroll-mt-[72px] space-y-4 pt-10"
          >
            <SectionHeading title="Beauty" />
            <div className="grid gap-5 md:grid-cols-2">
              {beauty.map((r) => (
                <ResourceCard
                  key={r.href}
