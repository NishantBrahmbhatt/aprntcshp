...
const SECTION_IDS = {
  general: "agriculture-section-general",
  environmental: "agriculture-section-environmental",
  animalCare: "agriculture-section-animal-care",
} as const;

const SECTION_PILLS: { id: string; label: string }[] = [
  { id: SECTION_IDS.general, label: "General" },
  { id: SECTION_IDS.environmental, label: "Environmental & Sustainability" },
  { id: SECTION_IDS.animalCare, label: "Animal Care" },
];
...
function PageHeader() {
  return (
    <section className="pt-8 pb-10">
      <IndustryBreadcrumb industryLabel="Agriculture, Environmental & Animal Care" />
      <div className="space-y-3">
        ...
        <p className="text-sm sm:text-base text-neutral-500 max-w-2xl">
          Everything you need to pursue an agriculture, environmental or animal
          care apprenticeship.
        </p>
      </div>
    </section>
...
function AgricultureSectionsNav({
  activeSectionId,
  onPillClick,
}: {
  ...
  return (
    <nav
      ...
      aria-label="Agriculture, environmental and animal care guide sections"
...
export default function AgricultureIndustryPage() {
  ...
  const environmentalRef = useRef<HTMLElement | null>(null);
  const animalCareRef = useRef<HTMLElement | null>(null);
      ...
      <main className="mx-auto flex min-h-screen max-w-7xl flex-col px-6 pt-5 pb-20">
        ...
        <div className="flex-1">
          ...
          <section
            ...
            id={SECTION_IDS.environmental}
            className="scroll-mt-[72px] space-y-4 py-10"
          >
            <SectionHeading title="Environmental & Sustainability" />
            <div className="grid gap-5 md:grid-cols-2">
              {environmentalAndSustainability.map((r) => (
                <ResourceCard
                  key={r.href}
                  title={r.title}
                  source={r.source}
          ...
          <section
            ref={animalCareRef}
            id={SECTION_IDS.animalCare}
            className="scroll-mt-[72px] space-y-4 pt-10"
          >
            <SectionHeading title="Animal Care" />
            <div className="grid gap-5 md:grid-cols-2">
              {animalCare.map((r) => (
                <ResourceCard
                  key={r.href}
        ...
        <PageRating pageName="agriculture" />
        <SiteFooter />
      </main>
    </div>
  );
