...
import {
  ...
  facilitiesAndFood,
  general,
} from "@/lib/data/industries/catering-hospitality";

const SECTION_IDS = {
  general: "catering-hospitality-section-general",
  events: "catering-hospitality-section-events",
  facilities: "catering-hospitality-section-facilities",
} as const;

const SECTION_PILLS: { id: string; label: string }[] = [
  { id: SECTION_IDS.general, label: "General" },
  { id: SECTION_IDS.events, label: "Events & Travel" },
  { id: SECTION_IDS.facilities, label: "Facilities & Food" },
];
...
function PageHeader() {
  return (
    <section className="pt-8 pb-10">
      <IndustryBreadcrumb industryLabel="Catering & Hospitality" />
      <div className="space-y-3">
        ...
        <p className="text-sm sm:text-base text-neutral-500 max-w-2xl">
          Everything you need to pursue a catering or hospitality apprenticeship.
        </p>
      </div>
    </section>
  );
...
function CateringHospitalitySectionsNav({
  activeSectionId,
  onPillClick,
}: {
  ...
  return (
    <nav
      ...
      aria-label="Catering and hospitality guide sections"
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-neutral-50">
      <main className="mx-auto flex min-h-screen max-w-7xl flex-col px-6 pt-5 pb-20">
        ...
        <div className="flex-1">
          ...
          <CateringHospitalitySectionsNav
          ...
          >
            <SectionHeading title="Events & Travel" />
            <div className="grid gap-5 md:grid-cols-2">
              {eventsAndTravel.map((r) => (
                <ResourceCard
                  key={r.href}
                  title={r.title}
                  source={r.source}
          ...
          <section
            ref={facilitiesRef}
            id={SECTION_IDS.facilities}
            className="scroll-mt-[72px] space-y-4 pt-10"
          >
            <SectionHeading title="Facilities & Food" />
            <div className="grid gap-5 md:grid-cols-2">
              {facilitiesAndFood.map((r) => (
                <ResourceCard
                  key={r.href}
                  title={r.title}
                  source={r.source}
        ...
        <PageRating pageName="catering-hospitality" />
        <SiteFooter />
      </main>
    </div>
  );
